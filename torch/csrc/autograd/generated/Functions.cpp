...
variable_list BtrisolveBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  if (should_compute_output({ self_ix })) {
...
variable_list CumprodBackward1::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = cumprod_backward(grad, self, dim, dtype);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CumsumBackward1::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = cumsum_backward(grad, dim, self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list IndicesBackward0::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
...
variable_list IndicesBackward1::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
...
variable_list IndexBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix })) {
    auto grad_result = self_info.zeros().index_put_(indices, grad, true);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list IndexPutBackward::apply(variable_list&& grads) {
  ...
  auto indices = unpack_list(indices_);
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.clone().index_put_(indices, values_info.zeros(), accumulate);
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list MeanBackward1::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.expand(self_sizes).to(self.scalar_type()) / self_numel;
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list MeanBackward2::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
...
variable_list MeanBackward3::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, false).to(self.scalar_type()) / _safe_size(self_sizes, dim);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list MeanBackward4::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, keepdim).to(self.scalar_type()) / _safe_size(self_sizes, dim);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list NormBackward2::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = norm_backward(grad, self.to(grad.scalar_type()), p, result).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list NormBackward3::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = norm_backward(grad, self.to(grad.scalar_type()), p, result, dim, keepdim).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CdistBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ x2_ix })) {
    auto grad_result = _cdist_backward(grad.t().contiguous(), x2, x1, p, result.t().contiguous());
...
variable_list PotriBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  if (should_compute_output({ self_ix })) {
...
variable_list ProdBackward1::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.scalar_type()), result).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list ProdBackward2::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
...
variable_list ProdBackward3::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.scalar_type()), result, dim, false).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list ProdBackward4::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.scalar_type()), result, dim, keepdim).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SumBackward1::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.expand(self_sizes).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list SumBackward2::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
...
variable_list SumBackward3::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, false).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list SumBackward4::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, keepdim).to(self.scalar_type());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list TriangularSolveBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix, A_ix })) {
    ...
    auto grad_result = triangular_solve_backward(grads[0], grads[1], self, A, result0, upper, transpose, unitriangular, grad_input_mask);
      if (should_compute_output({ self_ix })) {
        copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
      }
      if (should_compute_output({ A_ix })) {
...
variable_list SparseMaskBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ mask_ix })) {
    auto grad_result = not_implemented("sparse_mask");
...
variable_list ValuesBackward1::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
...
variable_list EmbeddingBagBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ weight_ix })) {
    auto grad_result = _embedding_bag_backward(grad, indices, offsets, result1, result2, result3, weight_argsize_0, scale_grad_by_freq, mode, sparse);
    copy_range(grad_inputs, weight_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ThnnConvTranspose2DBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
  auto bias_ix = gen.range(1);
...
variable_list ThnnConvTranspose2DBackwardBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
...
variable_list ThnnConvTranspose3DBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
  auto bias_ix = gen.range(1);
...
variable_list ThnnConvTranspose3DBackwardBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
...
variable_list ThnnConvDilated2DBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
  auto bias_ix = gen.range(1);
...
variable_list ThnnConvDilated2DBackwardBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
...
variable_list ThnnConvDilated3DBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
  auto bias_ix = gen.range(1);
...
variable_list ThnnConvDilated3DBackwardBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  auto self_ix = gen.range(1);
  auto weight_ix = gen.range(1);
...
variable_list ThnnCol2ImBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  if (should_compute_output({ self_ix })) {
    auto grad_result = thnn_col2im_backward(grad, kernel_size, dilation, padding, stride);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list ThnnIm2ColBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  if (should_compute_output({ self_ix })) {
    auto grad_result = thnn_im2col_backward(grad, {self_argsize_2, self_argsize_3}, kernel_size, dilation, padding, stride);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
